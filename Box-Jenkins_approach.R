...
x <- ts(train[c(2,3,5)], start = c(1987, 1), end = c(2010, 12), frequency=12)
y <- ts(train[4], start = c(1987, 1), end = c(2010, 12), frequency=12)
plot(x)
plot(y)
View(x)
View(y)
...
plot(AP1.60)
acf(AP1.60, lag.max = 288)
adf.test(AP1.60, k = 20)

AP1.60.2 <- diff(AP1.60, lag = 60)
plot(AP1.60.2)
acf(AP1.60.2, lag.max = 288)
adf.test(AP1.60.2, k = 20)

# Choose d=1, D=2, m=60. Check ACF and PACF to choose p, q, P, Q.
acf(AP1.60.2, lag.max = 167)
pacf(AP1.60.2, lag.max = 167)

# Try p = 11, q = 1, P = Q = 1
# m1.ml <- arima(log(y), order = c(11,1,1), seasonal = list(order = c(1,2,1), period = 60), method = "CSS-ML")
# m1.ml
# Try p = 2, q = 1, P = Q = 1
m2.ml <- arima(log(y), order = c(2,1,1), seasonal = list(order = c(1,2,1), period = 60), method = "CSS-ML")
m2.ml
# Try p = 11, q = 12, P = Q = 2
# m3.ml <- arima(log(y), order = c(11,1,12), seasonal = list(order = c(2,2,2), period = 60), method = "CSS-ML")
# m3.ml
# Try p = 2, q = 12, P = Q = 2
# m4.ml <- arima(log(y), order = c(2,1,12), seasonal = list(order = c(2,2,2), period = 60), method = "CSS-ML")
# m4.ml
# Try p = 2, q = 1, P = Q = 2
# m5.ml <- arima(log(y), order = c(2,1,1), seasonal = list(order = c(2,2,2), period = 60), method = "CSS-ML")
# m5.ml
# Check auto.arima
...
auto.m2 <- arima(log(y), order = c(2,1,1), seasonal = list(order = c(2,0,0), period = 12), method = "CSS-ML")
auto.m2
